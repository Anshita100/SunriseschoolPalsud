// src/components/boarding/BoardingSidebar.jsx
...
export default function BoardingSidebar() {
  const location = useLocation();

  const formatPath = (sub) =>
    `/boarding/${sub.toLowerCase().replace(/\s+/g, "-")}`;

  return (
    <aside className="w-full md:w-80 px-4 py-6 space-y-6">
      {/* Box 1: Boarding Navigation */}
      <div className="bg-white shadow-xl rounded-lg p-4">
        <h2 className="text-xl font-bold mb-4 border-b pb-2 text-primary-600">BOARDING</h2>
        <ul className="space-y-2">
          {boardingLinks.map((link) => (
            <li key={link}>
              <Link
                to={formatPath(link)}
